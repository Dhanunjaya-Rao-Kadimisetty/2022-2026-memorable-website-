...
async function fetchProfileRows(admin: ReturnType<typeof ensureAdminClient>) {
  ...
  if (/whatsapp_url|instagram_url|snapchat_url|twitter_url|birthday|photo_alignment/i.test(withPriorityPhotos.error.message ?? '')) {
    return admin
      .from('profiles')
...
async function fetchGalleryRows(admin: ReturnType<typeof ensureAdminClient>) {
  ...
  if (/audio_path/i.test(withTags.error.message ?? '')) {
    return admin
      .from('gallery_images')
...
export async function GET() {
  try {
    ...
    if (messagesResponse.error) {
      ...
      return jsonNoStore({ error: messagesResponse.error.message }, 500);
    }
