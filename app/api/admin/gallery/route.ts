...
async function insertGalleryRow(
  admin: Awaited<ReturnType<typeof ensurePublicBucket>>,
  values: {
    title: string;
    category: string;
...
export async function POST(request: Request) {
  try {
    ...
    if (!(media instanceof File) || media.size === 0) {
      return NextResponse.json({ error: 'gallery media file is required' }, { status: 400 });
    }
    ...
    if (responseData?.id) {
      await syncGalleryTags(admin, responseData.id, validTaggedProfileIds);
    }
